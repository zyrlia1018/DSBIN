...
def main():
    train_ligand_graphs = load_graphs('../lig_2016_trainset_graph_fea3')[0]
    train_protein_graphs = load_graphs('../pro_2016_trainset_graph_fea3')[0]
    train_pkd = np.load('../train_labels_2016.npy')

    test_ligand_graphs = load_graphs('../lig_2016_testset_graph_fea3')[0]
    test_protein_graphs = load_graphs('../pro_2016_testset_graph_fea3')[0]
    test_pkd = np.load('../test_labels_2016.npy')
    
    # Norm = Nomalizer(train_pkd)
    # train_pkd = Norm.norm(train_pkd)
    # test_pkd = Norm.norm(test_pkd)
    del train_ligand_graphs[2824]
    del train_protein_graphs[2824]
    train_pkd = train_pkd.tolist()
    del train_pkd[2824]
    train_pkd = np.array(train_pkd)
    ...
    n_feat_dim = int(len(train_ligand_graphs[0].ndata['x'].float()))
    e_feat_dim = int(len(train_ligand_graphs[0].edata['w'].float()))

    model = DSBINModel(node_input_dim=n_feat_dim, edge_input_dim=e_feat_dim,
                       node_hidden_dim=n_feat_dim, edge_hidden_dim=n_feat_dim,
                       num_step_message_passing=3, interaction=interaction)
    model.to(device)
