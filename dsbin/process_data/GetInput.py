...
def files():
    root_dir_path = "../data/process_refine2016/"
    ...
    protein_files = [os.path.join(root_dir_path, "{}/".format(pdb), "{}_proteinNOH2O.pdb".format(pdb)) for pdb in PDB_code]
...
def splitdata_core(PDB_code, LIG_G, PRO_G, labels):
    ...
    core_discompair = []
    all = []
    with open("../data/pdbbind_v2019_refined/2016coreset.txt", "r",encoding='gbk') as f:
...
def main():
    ...
    #得到了具有atom性质&距离embedding的配体图/配体的坐标（refineset）
    LIG_G = AtomCoord_LIG_G(ligand_files)
    #得到atom性质embedding的蛋白图/蛋白的坐标(refineset)
    ...
    np.save('train_labels_2016_NOH2O',train_labels)
    np.save('test_labels_2016_NOH2O',test_labels)
    #保存图
    save_graphs('lig_2016_trainset_graph_NOH2O', lig_2016_trainset_graph)
    save_graphs('pro_2016_trainset_graph_NOH2O', pro_2016_trainset_graph)
    save_graphs('lig_2016_testset_graph_NOH2O', lig_2016_coreset_graph)
    save_graphs('pro_2016_testset_graph_NOH2O', pro_2016_coreset_graph)
    #保存坐标文件
...
#LIG_G,lig_coord = Atom_LIG_G(ligand_files)


# save_graphs('lig_2016_refineset_graph',LIG_G)
# save_graphs('pro_2016_refineset_graph',PRO_G)
# lig_coordinates = np.array(lig_coordinates)
# pro_coordinates = np.array(pro_coordinates)
# np.save('lig_coordinates',lig_coordinates)
# np.save('pro_coordinates',pro_coordinates)


#
#core_label = np.load('core_index_2016.npy').tolist()

# 根据索引取值
# lab = [labels[i]for i in index]
